...
impl<'a> Parser<'a> {
    ...
    fn convert_token_to_node(&mut self, left_expression: Node) -> Result<Node, ParseError> {
        match self.current_token {
            Token::Add => {
                self.get_next_token()?;
                // Get right side expression
                let right_side = self.generate_ast(OperPrec::AddSub)?;
                Ok(Node::Add(Box::new(left_expression), Box::new(right_side)))
            }
            Token::Subtract => {
                self.get_next_token()?;
                let right_side = self.generate_ast(OperPrec::AddSub)?;
            ...
            Token::Multiply => {
                self.get_next_token()?;
                // Get right side expression
                let right_side = self.generate_ast(OperPrec::MulDiv)?;
                Ok(Node::Multiply(
                    Box::new(left_expression),
                    Box::new(right_side),
                ))
            ...
            Token::Caret => {
                self.get_next_token()?;
                let right_side = self.generate_ast(OperPrec::Power)?;
                Ok(Node::Caret(Box::new(left_expression), Box::new(right_side)))
            }
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match &self {
            self::ParseError::UnableToParse(e) => {
                write!(f, "Error in evaluating, {}", e)
            }
            self::ParseError::InvalidOperator(e) => {
                write!(f, "Error in evaluating, {}", e)
            }
        }
    }
